  ...
  return (
    <nav data-scrolled={scrolled} className="sticky top-0 z-[100] flex items-center justify-between border-b border-[var(--line)] bg-[rgba(250,250,247,0.85)] px-14 py-[18px] backdrop-blur-[16px] transition-[padding] duration-300 data-[scrolled=true]:py-[14px] max-md:px-6 max-md:py-[14px]">
      ...
      <ul className="flex list-none gap-9 max-md:hidden">
        {NAV_LINKS.map((l) => (
          <li key={l.href}>
            <NavLink href={l.href} label={l.label} active={activeId === l.id} />
          </li>
        ))}
      </ul>
...
function NavLink({ href, label, active = false }: { href: string; label: string; active?: boolean }) {
  return (
    <a href={href} data-active={active} className="group/nav relative py-1 text-[13.5px] font-medium text-[var(--text-sec)] transition-colors duration-200 hover:text-[var(--text)] data-[active=true]:text-[var(--text)]">
      {label}
      <span className={`absolute bottom-0 left-0 h-[1px] bg-[var(--accent)] transition-[width] duration-300 group-hover/nav:w-full ${active ? "w-full" : "w-0"}`} />
    </a>
  );
}
